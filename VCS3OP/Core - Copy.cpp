...
void Core::viewDecoding()
{
	...
	printf("\nEstimated time run (in second) %.5f: ", duration);
	cout << endl;
	cout << "\n> Finished" << endl;
	cout << "\n" ;
	system("pause");
...
void Core::splitParam(string binParam1, string binParam2, Share& S1, Share& S2){
	...
	binAlpha1		= binParam1.substr(0, 224);
	binBeta1		= binParam1.substr((224), 224);
	binGamma1		= binParam1.substr((224 * 2), 224);
	binG			= binParam1.substr((224 * 3), 2048);
	binP1			= binParam1.substr((2048 + (224 * 3)), 1024);

	string binAlpha2, binBeta2, binGamma2, binH, binP2;

	binAlpha2		= binParam2.substr(0, 224);
	binBeta2		= binParam2.substr((224), 224);
	binGamma2		= binParam2.substr((224 * 2), 224);
	binH			= binParam2.substr((224 * 3), 2048);
	binP2			= binParam2.substr((2048 + (224 * 3)), 1024);
...
void Core::getCanvas(Share& S1, Share& S2){
	...
	if(S1.nP[0].x > S2.rotNP[0].x){
		if((S1.nP[0].x - S2.rotNP[0].x + (S2.width/2)) < (S1.width/2)){
			widthCanvas = (S1.width/2);
		} else {
			widthCanvas	= S1.nP[0].x - S2.rotNP[0].x + (S2.width/2);
		}
		S2x			= S1.nP[0].x - S2.rotNP[0].x;
		S1x			= 0;
	} else if(S1.nP[0].x < S2.rotNP[0].x){
		if((S2.rotNP[0].x - S1.nP[0].x + (S1.width/2)) < (S2.width/2)){
			widthCanvas = (S2.width/2);
		} else {
	...
	if(S1.nP[0].y > S2.rotNP[0].y){
		if((S1.nP[0].y - S2.rotNP[0].y + (S2.height/2)) < (S1.height/2)){
			heightCanvas	= (S1.height/2);
		} else {
			heightCanvas	= S1.nP[0].y - S2.rotNP[0].y + (S2.height/2);
		}
		S2y			= S1.nP[0].y - S2.rotNP[0].y;
		S1y			= 0;
	} else if(S1.nP[0].y < S2.rotNP[0].y){
		if((S2.rotNP[0].y - S1.nP[0].y + (S1.height/2)) < (S2.height/2)){
			heightCanvas	= (S2.height/2);
		} else {
...
void Core::stackShares2Canvas(Share S1, Share S2){
	...
	for(int i = 0; i < (heightCanvas * 2); i++){
		positionx = 0;
		for(int j = 0; j < (widthCanvas * 2); j++){
			if((i >= (S2.pointStart.y * 2)) && (i < ((S2.pointStart.y*2) + (S2.height)))){
				if((j >= (S2.pointStart.x * 2)) && (j < ((S2.pointStart.x*2) + (S2.width)))){
